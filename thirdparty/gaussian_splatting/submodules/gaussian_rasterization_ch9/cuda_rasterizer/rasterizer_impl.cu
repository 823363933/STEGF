...
// Forward-style visibility/binning pass that accumulates per-Gaussian
// alpha-compositing contribution into image-wide and unreliable-mask buffers.
int CudaRasterizer::Rasterizer::contribution(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
