...
This repository starts from the V2.6.4 experiment state of our project, built on top of STGS. The first committed version keeps the original STGS dynamic Gaussian representation as the main backbone and adds a static multi-level Euler grid for 6D appearance residual correction before rasterization.

V2.6.4 default settings:
- Static Euler grid appearance residual is enabled.
- Dynamic grid is disabled.
- EMS main guided sampling is disabled.
- Global omega / rotation split freeze is disabled.
- Temporal refine and staged dynamic routing are disabled.

The original STGS implementation and citation information are kept below for reference.
